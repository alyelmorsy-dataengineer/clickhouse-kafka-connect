 ...
 * naiveTimestampZones lets a specific "table.column" be reinterpreted in its true source zone
...
public class ClickHouseWriterNaiveTimestampZoneTest {
    ...
    private static long writeAndReadEpochMillis(ClickHouseWriter writer, Column column, Date value, String tableName) throws Exception {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        writer.doWriteColValue(column, out, new Data(Schema.OPTIONAL_INT64_SCHEMA, value), false, tableName);
        return ByteBuffer.wrap(out.toByteArray()).order(ByteOrder.LITTLE_ENDIAN).getLong();
    }

    @Test
    public void reinterpretsConfiguredNaiveTimestampColumnInItsSourceZone() throws Exception {
        Map<String, String> props = new HashMap<>();
        props.put(ClickHouseSinkConfig.NAIVE_TIMESTAMP_ZONES, "bo_invoice.CreateDate=Africa/Cairo");
        ...
        long writtenMillis = writeAndReadEpochMillis(writer, column, mislabeledAsUtc, "bo_invoice");
    ...
    @Test
    public void leavesUnconfiguredNaiveTimestampColumnUnchanged() throws Exception {
        ...
        long writtenMillis = writeAndReadEpochMillis(writer, column, value, "bo_invoice");
